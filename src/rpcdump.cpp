...
UniValue importprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"BTCNprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"BTCNprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
UniValue dumpprivkey(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"BTCNaddress\"\n"
            "\nReveals the private key corresponding to 'BTCNaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"BTCNaddress\"   (string, required) The BTCN address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const UniValue& params, bool fHelp)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by BitcoinNew %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
...
UniValue bip38encrypt(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "bip38encrypt \"BTCNaddress\"\n"
            "\nEncrypts a private key corresponding to 'BTCNaddress'.\n"
            "\nArguments:\n"
            "1. \"BTCNaddress\"   (string, required) The BTCN address for the private key (you must hold the key already)\n"
